...
import com.amazuzu.chess.services.FiguresCheck
...
/**
  ...
  */
class ChessGameYetAnotherSpec extends Specification with FiguresCheck {
  /*"chess game service" should {

    "give 1 result for 1x1 desk and 1 figure" in {
      ChessGameYetAnotherService(1, 1, List('N')).variants().toList === List(Figure(Point.Zero, 'N')) :: Nil
    }

    "give 0 result for 1x1 desk and 0 figures" in {
      ChessGameYetAnotherService(1, 1, List()).variants().toList === List(Nil)
    }

    "give 2 results for 3x2 desk and 2 kings" in {
      val chess = ChessGameYetAnotherService(3, 2, List('N', 'Q'))
      val variants = chess.variants()

      expectNReturn2(variants, 2) === Set(
    ...
    "give 4 results for 3x3 desk, 2 kings and 1 rook, case N R N" in {
      val chess = ChessGameYetAnotherService(3, 3, List('N', 'R', 'N'))
      val variants = chess.variants()
    ...
    "give 4 results for 3x3 desk, 2 kings and 1 rook, case R N N" in {
      val chess = ChessGameYetAnotherService(3, 3, List('R', 'N', 'N'))
      val variants = chess.variants()

      expectNReturn2(variants, 1) === Set(
        Set(Figure(Point(1, 0), 'R'), Figure(Point(0, 2), 'N'), Figure(Point(2, 2), 'N'))
      )
    }

    "give 4 results for 3x3 desk, 2 kings and 1 rook, case N N R" in {
      val chess = ChessGameYetAnotherService(3, 3, List('N', 'N', 'R'))
      val variants = chess.variants()
    ...
    "give 3 results for 4x1 desk, 2 kings" in {
      val chess = ChessGameYetAnotherService(4, 1, List('N', 'N'))
      val variants = chess.variants()

      expectNReturn2(variants, 3) === Set(
    ...
    "give correct results on 3x2 and 2 kings case" in {
      val chess = ChessGameYetAnotherService(3, 2, List('N', 'N'))
      val variants = chess.variants()
